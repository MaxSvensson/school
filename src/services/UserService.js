...
class UserService {
    ...
    async CreateSessionToken(user) {
        const token = await jwt.sign({ id:user._id }, "bF]%~P(}ReDqkDu'DhTB-Udp>H)/`z5b;-n/aP_qwJs<grm`SeXux%t(<Zx-de3{4");
        return token
    }

    async VerifySessionToken(token) {
        try {
            var decoded = await jwt.verify(token, "bF]%~P(}ReDqkDu'DhTB-Udp>H)/`z5b;-n/aP_qwJs<grm`SeXux%t(<Zx-de3{4");
